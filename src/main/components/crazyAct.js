...
var crazyProfile = (function(){
    ...
    var crazyInfo = obj.get('.crazy-info');
    var crazyBioCount = 0;
    function crazyText(){
        var bio = '姓名:宮本綠<br>英文:Miyamoto Green<br>出生於一九零一,一百一十七的高齡,經歷過二次世界大戰的男人.';
        if(crazyBioCount <= bio.length){
            crazyInfo.innerHTML = bio.substring(0,crazyBioCount);
            crazyBioCount++;
        } else{
            window.clearInterval(crazyId);
        }
    }
