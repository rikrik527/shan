...
module.exports.raining = function() {
    var sky = document.querySelector('.shan-li-horse-effect12');
    var drop = 1;
    var rainBox = [];
    var skylength = sky.getBoundingClientRect().width();
    for (var i = 0; i < skylength.width; i++) {
        var div = document.createElement('div')
        div.classList.add('rain')
        rainBox.push(div);
        rainBox.width = sky.width;
        drop++;
        div.style.top += drop;
        sky.appendChild(div)
    }
...
function render() {
    shanLiBc.innerHTML = shan.shanShow;
    requestId = window.requestAFrame(render);
}

function renderEx() {
    shanLiBcEx.innerHTML = shan.shanExShow;
    requestIdEx = window.requestAFrame(renderEx);
}
...
window.requestAFrame = (function() {
    return window.requestAnimationFrame ||
        ...
        window.oReuestAnimationFrame ||
        function(callback) {
            return window.setTimeout(callback, 1000 / 60); // shoot for 60fps
        }
})();
