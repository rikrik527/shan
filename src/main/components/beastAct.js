...
var beastProfile = (function(){
    ...
    var beastInfo = obj.get('.beast-info');
    var beastBioCount = 0;
    function beastText(){
        var bio = '姓名:野獸<br>英文:The Beast<br>出生於一九七七,推舉一千公斤的怪獸,是前任世界摔角冠軍,千噸重的大鐵鎚是他最拿手的犀利武器,';
        if(beastBioCount <= bio.length){
            beastInfo.innerHTML = bio.substring(0,beastBioCount);
            beastBioCount++;
        } else{
            window.clearInterval(beastId);
        }
    }
    var beastId = window.setInterval(beastText,100);
    beastText();
        }
    }
...
module.exports.beastFishEye = function() {
    ...
    beastBtn.insertAdjacentHTML('afterbegin', '<article class="beast-boxcontrol1"></article>');
    var beastBc = obj.get('.beast-boxcontrol1');
    beastBc1.innerHTML = beastFish;
}
